...
static int cache[MAX_NUMBER_OF_HOUSE][NUMBER_OF_COLOR][NUMBER_OF_COLOR];
static bool cached[MAX_NUMBER_OF_HOUSE][NUMBER_OF_COLOR][NUMBER_OF_COLOR];
...
static int paint_house(int index, int prev_color, int first_color)
{
	...
	for (int color = RED; color < NUMBER_OF_COLOR; color++)
	{
		...
		if (index == number_of_houses - 1 && color == first_color)
		{
			continue ;
		}

		int cost = costs[index][color] + paint_house(index + 1, color, first_color);

		minimum_cost = (minimum_cost == -1 ? cost : std::min(minimum_cost, cost));
	}

	cache[index][prev_color][first_color] = minimum_cost;
